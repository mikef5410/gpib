...
#N1000 signals errors a little differently
# sub EAV {
#   my $self = shift;
#   my $stb  = shift || undef;
...
sub iOPC {
  ...
  if ($timeout) {
    ...
    $self->log('KeysightN1000.IOTrace')->error( shortmess("IOPC Timeout") );
...
#  Control the internal lowpass filter and a divider in the N1000A trigger
#  DIV mode only for N1000C with opt 001, from 3GHz to 13 GHz
...
#  With N1000C option 001, if the pattern lock is ON together with the Eye/Mask,
